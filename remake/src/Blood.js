...
export class Blood extends Component {
    constructor(props){
        super(props);
        this.browsrec=this.browsrec.bind(this);
        this.state={
            city:'',
            Blood:'',
            data:[]
    ...
    browsrec= e =>{
        ...
        console.log("wedjwelkjwe");
        const pp=rootref.orderByChild('city').equalTo(this.state.city);
        pp.on('value',(snapshot)=>{
            ...
            for (let dd in rr){
                newrec.push({
                    ...
                    blood:rr[dd].Blood,
                    email:rr[dd].Email,
                    address:rr[dd].Address,
                    phone:rr[dd].Phone
                })
                this.setState({data:newrec});
                console.log(newrec);
    ...
    render() {
        return (
            <div className="waa">
            <img src="searchicon.png" className="iconic"></img>
            <img src="nsF97trg0D.jpg" className="mage"></img>
            <a href="/"><img src="logo.png" className="wi21"></img></a>
           
            <div className="vs">
                 <div>
                 <h1 className="vas">BLOOD DONORS</h1>
           <select className="bm" onChange={e=>this.setState({Blood:e.target.value})}>
          ...
          <select className="bmw" onChange={e=>this.setState({city:e.target.value})}>
...
<button className="aq" onClick={this.browsrec}>Submit</button>
</div><br></br><br></br><br></br><br></br>
        </div>
        {this.state.data.map((rrr)=>{
        if(rrr.blood==this.state.Blood){
        return(
        <div className="vo">
        <table className="vgh">
            <tr>
            ...
            <tr className="mukesh">
                <td><Link to={`/profile/${rrr.id}`} style={{textDecoration:'none',color:"black"}}>{rrr.name}</Link></td>
                <td><Link to={`/profile/${rrr.id}`} style={{textDecoration:'none',color:"black"}}>{rrr.blood}</Link></td>
                <td><Link to={`/profile/${rrr.id}`} style={{textDecoration:'none',color:"black"}}>{rrr.phone}</Link></td>
                <td><Link to={`/profile/${rrr.id}`} style={{textDecoration:'none',color:"black"}}>{rrr.email}</Link></td>
                <td><Link to={`/profile/${rrr.id}`} style={{textDecoration:'none',color:"black"}}>{rrr.address}</Link></td>
            </tr>
        </table>
        </div>)}})}
            </div>
